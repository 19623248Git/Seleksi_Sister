 ...
 * @brief The panel that handles drawing the Mandelbrot set.
 */
...
public:
    MandelbrotCanvas(wxWindow* parent) : wxPanel(parent)
    {
        ...
        // === CORRECTED: Use wxEVT_MOTION for mouse movement ===
...
private:
    ...
    void OnPaint(wxPaintEvent& event)
    {
        ...
        engine.generateMandelbrot(pixel_colors, width, height, minReal, maxReal, minImag, maxImag);
    ...
    void OnPanEnd(wxMouseEvent& event) {
        isPanning = false;
    }
    ...
    void OnMouseMove(wxMouseEvent& event) {
        if (!isPanning || !event.MiddleIsDown()) {
            return;
        }
        ...
        double deltaReal = delta.x * scale;
        double deltaImag = delta.y * scale;
...
public:
        ...
        : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, size)
    {
        ...
        applyButton->Bind(wxEVT_BUTTON, &MandelbrotFrame::OnApplySettings, this);

        OnApplySettings(wxCommandEvent());
    }
...
private:
    ...
    void OnApplySettings(const wxCommandEvent& event) 
    {
        long iterations;
...
public:
    virtual bool OnInit()
    {
        MandelbrotFrame* frame = new MandelbrotFrame("Interactive Mandelbrot Explorer", wxSize(1280, 720));
        frame->Show(true);
        frame->Center();
        return true;
    }
